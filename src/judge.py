"""Argument-structure judging via Gemini."""
...
3. 発言数の多さは加点理由にならない。
4. 参加者は、互いに直接やりとりを2回以上行っている2名のみを選ぶ。第三者のコメント・傍観者・話題に乗じた投稿は無視する。主要な2名が特定できない場合は valid を false にする。ハンドルは入力に出てきた文字列をそのまま使う。
...
def judge(transcript: str, api_key: str, model: str = "gemini-3.6-flash") -> dict[str, Any]:
    body = {
